"""
OHL Scraper Functions

Wrapper functions that convert OHL API responses to pandas DataFrames.
...
from scrapernhl.ohl.api import (
    ...
    get_play_by_play, get_game_summary, OHLConfig
)


def scrapeTeams(season: Union[int, str] = None, config: OHLConfig = None) -> pd.DataFrame:
    """
    Scrape OHL teams data as a DataFrame.
    
    Args:
        season: Season ID (uses default if None)
        config: OHLConfig instance
...
def scrapeSchedule(
    ...
    """
    Scrape OHL schedule as a DataFrame.
...
def scrapeScorebar(
    ...
    """
    Scrape OHL live games and recent results as a DataFrame.
...
def scrapeStandings(
    ...
    """
    Scrape OHL standings as a DataFrame.
...
def scrapeRoster(
    ...
    """
    Scrape OHL team roster as a DataFrame.
...
def scrapeSkaterStats(
    ...
    """
    Scrape OHL skater statistics as a DataFrame.
...
def scrapeGoalieStats(
    ...
    """
    Scrape OHL goalie statistics as a DataFrame.
...
def scrapePlayByPlay(game_id: int, config: OHLConfig = None) -> pd.DataFrame:
    """
    Scrape OHL play-by-play events as a DataFrame.
...
def scrapeGameSummary(game_id: int, config: OHLConfig = None) -> dict:
    """
    Scrape OHL game summary.
