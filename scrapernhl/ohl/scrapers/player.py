"""
OHL Player Scraper Module

This module provides functions to scrape comprehensive player data from the OHL API.
...
def scrape_player_career_stats(
    ...
    if isinstance(profile, dict) and 'careerStats' in profile:
        ...
        for i in range(len(career_data[0]['sections'])):
            ...
            # Only filter if the column exists and DataFrame has data
            if not df.empty and "row.season_name" in df.columns:
