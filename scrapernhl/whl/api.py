...
Comprehensive API for the Western Hockey League (WHL) using the HockeyTech platform.
Provides access to games, schedules, teams, rosters, player stats, and more.

The WHL uses the same LeagueStat platform structure as other HockeyTech leagues.
"""
...
class WHLConfig:
    """Configuration for WHL API access."""
    ...
    # Defaults
    DEFAULT_SEASON = "289"  # 2024-25 WHL season
...
@RateLimiter(calls=WHLConfig.RATE_LIMIT_CALLS, period=WHLConfig.RATE_LIMIT_PERIOD)
def fetch_api(
    feed: str,
    view: str,
...
def get_scorebar(
    ...
    if season is None:
        season = config.DEFAULT_SEASON if config else WHLConfig.DEFAULT_SEASON
...
def get_game_preview(game_id: int, config: WHLConfig = None) -> Dict[str, Any]:
...
def get_game_summary(game_id: int, config: WHLConfig = None) -> Dict[str, Any]:
...
def get_play_by_play(game_id: int, config: WHLConfig = None) -> List[Dict[str, Any]]:
...
def get_teams(season: Union[int, str] = None, config: WHLConfig = None) -> Dict[str, Any]:
...
def get_roster(
    ...
    if season_id is None:
        season_id = WHLConfig.DEFAULT_SEASON if config is None else config.DEFAULT_SEASON
...
def get_game_full_data(game_id: int, config: WHLConfig = None) -> Dict[str, Any]:
