...
from ..api import fetch_api, get_scorebar, PWHLConfig

# Schedule
@cached(ttl=3600, cache_key_func=lambda team_id=-1, season=None, month=-1, location='homeaway', **kwargs: f"pwhl_schedule_{team_id}_{season}_{month}_{location}")
def getScheduleData(team_id: int = -1, season: Union[int, str] = None, month: int = -1, location: str = 'homeaway') -> List[Dict]:
    """Scrapes raw PWHL schedule data."""
    if season is None:
        season = PWHLConfig.DEFAULT_SEASON
    console.print_info(f"Fetching PWHL schedule (team={team_id}, season={season})...")
    try:
        response = fetch_api(feed='statviewfeed', view='schedule', team=team_id, season=season, month=month, location=location, site_id=PWHLConfig.SITE_ID, conference_id=-1, division_id=-1)
        games = []
    ...
    return [{**record, "scrapedOn": now, "source": "PWHL Schedule API"} for record in games if isinstance(record, dict)]
...
def scrapeSchedule(team_id: int = -1, season: Union[int, str] = None, month: int = -1, location: str = 'homeaway', output_format: str = "pandas") -> Union[pd.DataFrame, pl.DataFrame]:
    """Scrapes PWHL schedule data."""
    raw_data = getScheduleData(team_id, season, month, location)
    return json_normalize(raw_data, output_format)

@cached(ttl=3600, cache_key_func=lambda team_id=None, season=None, **kwargs: f"pwhl_schedule_legacy_{team_id}_{season}")
def getScheduleLegacyData(team_id: Union[int, str] = None, season: Union[int, str] = None) -> List[Dict]:
    """
    Scrapes raw PWHL schedule data using the scorebar endpoint (legacy method).
    ...
    console.print_info(f"Fetching PWHL schedule via scorebar (team={team_id}, season={season})...")
    ...
    now = datetime.utcnow().isoformat()
    return [{**record, "scrapedOn": now, "source": "PWHL Scorebar API"} for record in games if isinstance(record, dict)]
...
def scrapeScheduleLegacy(team_id: Union[int, str] = None, season: Union[int, str] = None, output_format: str = "pandas") -> Union[pd.DataFrame, pl.DataFrame]:
    """
    Scrapes PWHL schedule data using the scorebar endpoint (legacy method).
    ...
    >>> df = scrapeScheduleLegacy(season=2)
    >>>
    >>> # Get schedule for specific team
    >>> df = scrapeScheduleLegacy(team_id=1, season=2)
    """
