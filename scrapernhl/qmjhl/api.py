...
Comprehensive API for the Quebec Maritimes Junior Hockey League (QMJHL) using the HockeyTech platform.
Provides access to games, schedules, teams, rosters, player stats, and more.

The QMJHL uses the same LeagueStat platform structure as other HockeyTech leagues.
"""
...
class QMJHLConfig:
    """Configuration for QMJHL API access."""
    ...
    # Defaults
    DEFAULT_SEASON = "211"  # 2024-25 QMJHL season
...
@RateLimiter(calls=QMJHLConfig.RATE_LIMIT_CALLS, period=QMJHLConfig.RATE_LIMIT_PERIOD)
def fetch_api(
    feed: str,
    view: str,
...
def get_scorebar(
    ...
    if season is None:
        season = config.DEFAULT_SEASON if config else QMJHLConfig.DEFAULT_SEASON
...
def get_schedule(
    ...
    if season is None:
        season = QMJHLConfig.DEFAULT_SEASON if config is None else config.DEFAULT_SEASON
...
def get_game_preview(game_id: int, config: QMJHLConfig = None) -> Dict[str, Any]:
...
def get_game_summary(game_id: int, config: QMJHLConfig = None) -> Dict[str, Any]:
...
def get_play_by_play(game_id: int, config: QMJHLConfig = None) -> List[Dict[str, Any]]:
...
def get_teams(season: Union[int, str] = None, config: QMJHLConfig = None) -> Dict[str, Any]:
...
def get_goalie_stats(
    ...
    Note: QMJHL API uses position='goalies' to filter for goalie-specific statistics
...
def get_game_full_data(game_id: int, config: QMJHLConfig = None) -> Dict[str, Any]:
